...
//--------------------------------- FUNÇÃO PARA PRODUTOS----------------------------------------//
        ...
        window.excluirProduto = async (id) => {
            const confirmacao = confirm("Tem certeza de que deseja excluir este produto e sua imagem?");
            if (!confirmacao) {
                return;
            }

            try {
                // 1. Primeiro busca o produto para obter a URL da imagem
                ...
                // 2. Se existir uma imagem, exclui do Storage
                if (produto.url_img) {
                    // Extrai o caminho do arquivo da URL completa
                    const urlObj = new URL(produto.url_img);
                    const filePath = urlObj.pathname.replace('/storage/v1/object/public/', '');

                    const { error: storageError } = await supabase
                        .storage
                        .from('assets') // Nome do seu bucket
                        .remove([filePath]);

                    if (storageError) console.warn("Aviso: Imagem não pôde ser excluída", storageError);
                }
                ...
                alert("Produto e imagem excluídos com sucesso!");
